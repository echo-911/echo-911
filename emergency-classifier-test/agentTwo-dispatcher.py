...
SEVERITY_LEVELS = {
    ...
    2: "Moderate priority, limited risk.",
    3: "Serious situation requiring attention.",
    4: "High priority with risk to people or property.",
    5: "Critical emergency, mass casualty or major threat."
}

def generate_emergency_audio(who_responding: list, main_incident: str, weather_info: str, severity: int, location: str) -> dict:
    """
    Generate audio file using AWS Polly for emergency dispatch information.
    
    Args:
        who_responding: List of responder objects
        main_incident: Description of the main incident
        weather_info: Current weather conditions
        severity: Severity level (1-5)
        location: Incident location
    
    Returns:
        Dictionary with audio file details
    """
    
    # Create responder summary
    responder_summary = []
    for responder in who_responding:
        unit = responder.get('unit', f"Unit {responder['id'][-3:]}")
        name = responder['name']
        resp_type = responder['type'].capitalize()
        eta = responder.get('eta_minutes', 'Unknown')
        responder_summary.append(f"{resp_type} {unit} with {name}, ETA {eta} minutes")
    
    responders_text = ". ".join(responder_summary)
    
    # Create the audio script with emergency details
    audio_script = f"""
    <speak>
        Emergency Response Summary for {location}.
        <break time="0.5s"/>
        ...
        Level {severity} out of 5. {SEVERITY_LEVELS.get(severity, 'Unknown priority level')}.
        <break time="1s"/>
        All units exercise appropriate caution and follow established protocols.
        End of emergency briefing.
    </speak>
    """
    
    try:
        # Generate speech using Polly with SSML
        ...
        audio_filename = f"emergency_dispatch_{timestamp}.mp3"
        ...
        if 'AudioStream' in response:
            ...
            return {
                ...
                "text_content": audio_script,
                "voice_used": "Matthew",
                "format": "mp3",
                "timestamp": timestamp,
                "responders_count": len(who_responding),
                "severity": severity,
                "location": location
            }
        else:
            return {"success": False, "error": "No audio stream in response"}
            
    except Exception as e:
        print(f"❌ Polly Audio Generation Error: {str(e)}")
        return {
            "success": False, 
            "error": str(e),
...
def main():
    ...
    # Example weather info string (replace with live data if available)
    current_weather = "Weather conditions are clear with calm wind, minimizing fire spread risk"

    incident = results['incident']

    response_audio = generate_emergency_audio(
        who_responding=results['assigned_responders'],
        main_incident=incident['description'],
