...
const geocodeAddress = (address, isDispatcher = false) => {
...
const EmergencyDashboard = () => {
  ...
  const [dispatcherLocation, setDispatcherLocation] = useState({
    lat: 32.8423, // some default dispatcher location
    lng: -96.7828
  });
  ...
  <p className="text-white/70 text-sm leading-relaxed">
    {`The caller reports visible flames from the second-floor windows and possible occupants trapped inside. The nearest fire station is approximately 2 miles away (~5 min response), and paramedics have been dispatched alongside the fire department. Weather conditions are clear with calm wind, minimizing fire spread risk.`}
          ...
          <div className="col-span-5 space-y-6">
            {/* Top Right - Map */}
            <div className="backdrop-blur-xl bg-white/6 border border-white/15 rounded-3xl p-3 h-50px] hover:scale-[1.02] transition-all duration-300">
...
>
  ...
  {dispatcherLocation && (
    <Marker
      ...
      title="Dispatcher (Fire Station)"
    />
  )}

  {/* User marker (hardcoded address) */}
  {location && (
    <Marker
      position={location}
  ...
  {directions && <DirectionsRenderer options={{ directions }} />}
            ...
            <div className="backdrop-blur-xl bg-white/6 border border-white/15 rounded-3xl p-6 h-[425px] hover:scale-[1.02] transition-all duration-300">
